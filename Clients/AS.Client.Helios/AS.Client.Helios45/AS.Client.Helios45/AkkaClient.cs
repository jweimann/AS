...
namespace AS.Client.Helios
{
    public class AkkaClient : IDisposable
    {
        ...
        private void ReceivedDataCallback(NetworkData incomingData, IConnection responseChannel)
        {
            ...
            if (message is BatchedMessage)
            {
                var messages = ((BatchedMessage)message).Messages;
                //Logger.LogWarning("ReceivedDataCallback BatchedMessage - MessageCount: {0}", messages.Length.ToString());
                for (int i = 0; i < messages.Length; i++)
                {
                    //Logger.LogWarning("Inner Message: {0}", messages[i].ToString());
